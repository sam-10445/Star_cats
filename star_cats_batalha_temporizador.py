...
nota_peixe = ""
nota_cao = ""
pontos_black1 = 50 #o valor deve ser zero
pontos_black2 = 50 #o valor deve ser zero
pontos_black3 = 50 #o valor deve ser zero
...
quadrados_batalha = []
quadrado_verde_index = -1
tempo_quadrado_verde = 0
DURACAO_QUADRADO_VERDE = 700  # ms
...
#criar grade 4x4 da batalha
for linha in range(GRID_SIZE):
    for coluna in range(GRID_SIZE):
        x = GRID_START_X + coluna * GRID_CELL
        y = GRID_START_Y + linha * GRID_CELL
        quadrados_batalha.append(pygame.Rect(x, y, GRID_CELL, GRID_CELL))
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            #clique no quadrado verde da batalha do passaro
            if event.button == 1 and em_batalha_passaro:
                if quadrado_verde_index != -1:
                    if quadrados_batalha[quadrado_verde_index].collidepoint(event.pos):
                        pontos_black1 += 10
                        quadrado_verde_index = -1
    ...
    if em_batalha_passaro:
        ...
        #MINI GAME DOS QUADRADOS
        agora = pygame.time.get_ticks()

        # escolher novo quadrado verde
        if quadrado_verde_index == -1 or agora - tempo_quadrado_verde > DURACAO_QUADRADO_VERDE:
            quadrado_verde_index = random.randint(0, len(quadrados_batalha) - 1)
            tempo_quadrado_verde = agora

        # desenhar grade
        for i, q in enumerate(quadrados_batalha):
            if i == quadrado_verde_index:
                pygame.draw.rect(janela, (0, 200, 0), q)  # verde
            else:
                pygame.draw.rect(janela, (0, 0, 0), q, 2)  # borda preta
